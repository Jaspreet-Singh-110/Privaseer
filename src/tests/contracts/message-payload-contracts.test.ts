...
describe('message payload contracts', () => {
  ...
  it('rejects invalid payloads for validated message types', async () => {
    const invalidCases: Array<{ type: MessageType; payload: unknown }> = [
      ...
      { type: 'REPORT_FALSE_POSITIVE', payload: { domain: 'x', url: '', detectedPatterns: 'bad', timestamp: '1', installationId: 1, scanConfidence: '0' } },
  ...
  it('accepts valid payloads for validated message types', async () => {
    const validCases: Array<{ type: MessageType; payload: unknown }> = [
      ...
      { type: 'REPORT_FALSE_POSITIVE', payload: { domain: 'example.com', url: 'https://example.com', detectedPatterns: [], timestamp: Date.now(), installationId: 'install-1', scanConfidence: 0.5 } },
